...
class FDTD():
    ...
    def __init__(self, f, xmin, xmax, ymin, ymax, nt, interval, src, p = 1, q = 1.5, R = 50):
        ...
        self.nx = int((xmax-xmin)/self.ds) 
        self.ny = int((ymax-ymin)/self.ds)
        self.p = p
        self.q = q
    ...
    def pml(self, eps, mu, nb = 10, m = 3, R0 = 10**-5):
        ...
        sigma_mx = np.zeros((self.nx, self.ny))
        sigma_my = np.zeros((self.nx, self.ny))
        sigmamax = -math.log(R0)*(m+1)/(np.sqrt(mu/eps)*2*nb*self.ds)/self.p
        for n in range(nb):
            sigma_x[n,:] = ((nb-n)/nb)**m*sigmamax[n,:]
            sigma_x[-n-1,:] = ((nb-n)/nb)**m*sigmamax[-n-1,:]
            sigma_mx[n,:] = ((nb-n+0.5)/nb)**m*sigmamax[n,:]
            sigma_mx[-n-1,:] = ((nb-n-0.5)/nb)**m*sigmamax[-n-1,:]

            sigma_y[:,n] = ((nb-n)/nb)**m*sigmamax[:,n]
            sigma_y[:,-n-1] = ((nb-n)/nb)**m*sigmamax[:,-n-1]
            sigma_my[:,n] = ((nb-n+0.5)/nb)**m*sigmamax[:,n]
            sigma_my[:,-n-1] = ((nb-n-0.5)/nb)**m*sigmamax[:,-n-1]

        return sigma_x, sigma_y, sigma_mx*mu/eps, sigma_my*mu/eps
